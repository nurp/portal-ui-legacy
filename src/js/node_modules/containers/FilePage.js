...
const FilePage = ({
  ...
  const filteredAE = node.associated_entities
  .filter(ae => Object.keys(ae).map(k => ae[k].includes(searchTerm)).includes(true))
  .map(ae => {
    const annotations = node.annotations.filter(a => a.entity_id === ae.entity_id);
      ...
      <Row style={{ alignItems: 'flex-start' }}>
        ...
        <EntityPageVerticalTable
          ...
          thToTd={[
            ...
            { th: 'Tag', td: JSON.parse((node.tags || '[--]').replace(/u'/g, '\'').replace(/'/g, "\""))},
          ]}
        />
      </Row>
