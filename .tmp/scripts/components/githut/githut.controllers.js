            ...
            (function (controllers) {
                var GitHutController = (function () {
                    ...
                    GitHutController.prototype.draw = function () {
                        ...
                        var svg = d3.select(options.container).append("svg").attr("width", WIDTH).attr("height", HEIGHT);
                        var defs = svg.append("defs").append("pattern").attr({
                            id: "diagonalHatch",
                            width: 3,
                            height: 3,
                        ...
                        var xscale = d3.scale.ordinal().domain(options.columns).rangePoints([
                            0,
                            WIDTH - (margins.left + margins.right + padding.left + padding.right)
                        ]);
                        var yscales = {}, width_scales = {}, extents = {}, yAxes = {};
                        var languages_group = svg.append("g").attr("id", "languages").attr("transform", "translate(" + (margins.left + padding.left) + "," + (margins.top + padding.top) + ")");
                        var labels_group = svg.append("g").attr("id", "labels").attr("transform", "translate(" + (margins.left + padding.left) + "," + (margins.top + padding.top) + ")");
                        var columns = svg.append("g").attr("id", "columns").attr("transform", "translate(" + (margins.left + padding.left) + "," + (margins.top + padding.top) + ")");
                        var labels = labels_group.selectAll("g.labels").data(nested_data, function (d) {
                            return d.key;
                        }).enter().append("g").attr("class", "labels").attr("rel", function (d) {
                            return d.key;
                        }).on("mouseover", function (d) {
                            d3.select(this).classed("hover", true);
                            languages_group.selectAll("g.lang[rel='" + d.key + "']").classed("hover", true);
                        }).on("mouseout", function (d) {
                            svg.selectAll("g.hover,g.primary_site").classed("hover", false).classed("primary_site", false);
                        });
                        var language = languages_group.selectAll("g.lang").data(nested_data, function (d) {
                            return d.key;
                        }).enter().append("g").attr("class", "lang").attr("rel", function (d) {
                            return d.key;
                        });
                        ...
                        function updateScales() {
                            ...
                            options.columns.forEach(function (d) {
                                ...
                                if (options.scale_map[d] === "ordinal") {
                                    var inc = 0.001;
                                    var domain = nested_data.filter(function () {
                                        return true;
                                    }).sort(function (a, b) {
                                    ...
                                    scales[d] = d3.scale.ordinal().domain(domain).rangePoints([HEIGHT - (margins.top + margins.bottom + padding.top + padding.bottom), 0]);
                                }
                                    ...
                                    var indices = primary_sites.map(function (d, i) {
                                        return i * yRange / (indices ? indices.length : 1);
                                    });
                        ...
                        function addAxes() {
                            var column = columns.selectAll("g.column").data(options.config).enter().append("g").attr("class", "column").attr("transform", function (d) {
                                var x = xscale(d.id);
                                return "translate(" + x + "," + 0 + ")";
                            });
                            var title = column.append("text").attr("class", "title").attr("x", 0).attr("y", 0);
                            ...
                            }).classed("first", true).attr("transform", "translate(-10,0)");
                            title.selectAll("tspan").data(function (d) {
                                return d.display_name;
                            }).enter().append("tspan").attr("x", 0).attr("y", function (d, i) {
                                // Magic numbers
                            ...
                            if (options.superhead) {
                                svg.append("g").attr("height", 100).attr("transform", function (d) {
                                    var x = xscale(options.superhead.start) + (0.5 * (xscale(options.superhead.end) - xscale(options.superhead.start))) + padding.left + margins.left, y = 10;
                                    return "translate(" + x + "," + y + ")";
                                }).append("text").text(options.superhead.text).style("text-anchor", "middle").attr("class", "title");
                                svg.append("rect").attr("width", function () {
                                    var width = xscale(options.superhead.end) - xscale(options.superhead.start);
                                    // no neg widths
                                    return width > 0 ? width : 0;
                                }).attr("height", 1).attr("y", 15).attr("x", xscale(options.superhead.start) + padding.left + margins.left);
                            }
                            var axis = column.filter(function (col) {
                                return options.scale_map[col] == "ordinal" && col != options.title_column;
                            }).append("g").attr("class", "axis").attr("transform", function (d) {
                            ...
                            var ticks = axis.selectAll("g.tick").data(function (d) {
                                var ticks = [
                                ...
                                return ticks;
                            }).enter().append("g").attr("class", "tick").classed("start", function (d) {
                                return d.x < 0;
                            ...
                            // Magic Numbers
                            ticks.append("line").attr("x1", 0).attr("y1", -3).attr("x2", 0).attr("y2", 3);
                            ticks.append("text").attr("x", 0).attr("y", 12).text(function (d) {
                                return d3.format("s")(d.value);
                            });
                        }
                        function updateAxes() {
                            columns.selectAll("g.axis").selectAll("g.tick").data(function (d) {
                                var ticks = [0, width_scales[d].domain()[1]].map(function (v, i) {
                                    return {
                                        value: i === 0 ? 0 : v,
                                        x: (i === 0 ? 0 : width_scales[d](v) / 2),
                                        domain: width_scales[d].domain(),
                        ...
                        function createLanguages(languages) {
                            ...
                            languages.append("g").attr("class", "lang-label").call(createLangLabel);
                        }
                        function updateMarkers(duration) {
                            var marker = languages_group.selectAll(".lang").select("g.markers").selectAll("g.marker").data(function (d) {
                            ...
                            marker.exit().remove();
                            var new_markers = marker.enter().append("g").attr("class", "marker").classed("ordinal", function (d) {
                                return options.scale_map[d.column] == "ordinal";
                            }).attr("transform", function (d) {
                                var x = xscale(d.column), y = yscales[d.column](d.value);
                                return "translate(" + x + "," + y + ")";
                            });
                            new_markers.filter(function (d) {
                                return options.scale_map[d.column] == "ordinal";
                            }).append("rect").attr("x", function (d) {
                                return 0;
                            }).attr("y", -4).attr("width", 0).attr("height", 8).style("fill", function (d) {
                                return options.color_groups[options.color_group_map[d.column]];
                            });
                            marker.transition().duration(duration || options.duration).attr("transform", function (d) {
                            ...
                            marker.select("rect").transition().duration(options.duration).attr("x", function (d) {
                                return -width_scales[d.column](d.value / ((options.dimensions.indexOf(d.column) > -1) ? 1 : d.ref)) / 2;
                            }).attr("width", function (d) {
                                var width = width_scales[d.column](d.value / ((options.dimensions.indexOf(d.column) > -1) ? 1 : d.ref));
                                // no neg widths
                                return width > 0 ? width : 0;
                            });
                        }
                        function updateConnections(duration) {
                            var connection = languages_group.selectAll(".lang").select("g.connections").selectAll("g.connection").data(function (d) {
                            ...
                            var new_connection = connection.enter().append("g").attr("class", "connection");
                            new_connection.append("path").attr("class", "hover");
                            new_connection.append("path").attr("class", "line");
                            var paths = ["line", "hover"];
                            paths.forEach(function (p) {
                                connection.select("path." + p).transition().duration(duration || options.duration).attr("d", function (d) {
                                    return line(d.path);
                                });
                            });
                        }
                        function updateLabels(duration) {
                            var labelAdjust = 25;
                            var labels = labels_group.selectAll(".labels").selectAll("g.label").data(function (d) {
                            ...
                            var new_label = labels.enter().append("g").attr("class", "label").classed("primary_site", function (d) {
                                return d.column === "primary_site";
                            });
                            new_label.filter(function (d) {
                                return d.column !== "primary_site" && d.column !== options.title_column;
                            }).append("path");
                            new_label.filter(function (d) {
                                return d.column !== "primary_site" && d.column !== options.title_column;
                            }).append("text").attr("x", 0).attr("y", 4);
                            new_label.filter(function (d) {
                                return d.column === "primary_site";
                            }).append("text").attr("x", -10).attr("y", 3).attr("class", "visible-always");
                            new_label.append("rect").attr("class", "ix").attr("y", -8).attr("height", 15).on("click", function (z, i, m) {
                                if (z.href && z.value) {
                                    z.href(z);
                                }
                            });
                            ...
                            labels.select("text").attr("x", function (d) {
                            ...
                            }).attr("transform", "translate(" + labelAdjust + ",0)").style("text-anchor", function (d) {
                                if (d.column === "primary_site") {
                                    return "start";
                                }
                                else {
                            ...
                            labels.select("path").attr("class", "label").attr("d", function (d) {
                                var dw = 10, w = d.text_width + dw;
                                return "M" + (w / 2 + dw / 2) + ",0l-" + dw / 2 + ",-10l-" + w + ",0l0,20l" + w + ",0z";
                            }).attr("x", 50).attr("transform", "translate(" + labelAdjust + ",0)");
                            labels.select("rect.ix").attr("x", function (d) {
                            ...
                            labels.filter(function (d) {
                                ...
                                labels_group.selectAll(".labels").classed("hover", function (l) {
                                    return l.values.primary_site == d.value;
                                });
                                languages_group.selectAll(".lang").classed("hover", function (l) {
                                    return l.values.primary_site == d.value;
                                });
                            });
                            var projectTip = d3.tip().attr("class", "tooltip").offset([-5, 0]).html(function (d) {
                                return _.find(nested_data, { key: d.lang }).values.name;
                            });
                            labels.filter(function (d) {
                                return d.column === "project_id";
                            }).call(projectTip).on("mouseover", projectTip.show).on("click", projectTip.hide).on("mouseout", projectTip.hide);
                            ...
                            labels.filter(function (d) {
                                ...
                                labels_group.selectAll(".labels").classed("hover", function (l) {
                                    ...
                                    // are highlighted
                                    if (ret) {
                                        languages_group.selectAll("g.lang[rel='" + l.key + "']").classed("hover", true);
                                    }
                                    return ret;
                                });
                            });
                        ...
                        function createLangLabel(lang_label) {
                            lang_label.attr("transform", function (d) {
                                var x = xscale(options.title_column), y = yscales[options.title_column].range()[0];
                                return "translate(" + x + "," + y + ")";
                            });
                            var rect = lang_label.append("rect").attr("x", -(padding.left + margins.left)).attr("width", padding.left + margins.left).attr("y", -9).attr("height", 16);
                            lang_label.append("text").attr("x", -10).attr("y", 3).text(function (d) {
                                return d.values[options.title_column];
                            });
                        }
                        function updateLangLabels(duration) {
                            languages_group.selectAll(".lang").select("g.lang-label").transition().duration(duration || options.duration).attr("transform", function (d) {
                ...
                angular.module("githut.controllers", []).controller("GitHutController", GitHutController);
