...
(function (ngApp) {
    ...
    (function (components) {
        ...
        (function (charts) {
            /* @ngInject */
            function PieChart($window, LocationService, $state) {
                return {
                    restrict: "EA",
                    replace: true,
                    scope: {
                    ...
                    link: function ($scope, element) {
                        var tip = d3.tip().attr("class", "tooltip").offset([-5, 0]).html(function (d) {
                        ...
                        function updateChart() {
                            ...
                            var arc = d3.svg.arc().padRadius(outerRadius).innerRadius(0);
                            var svg = d3.select(element.find(".chart-container")[0]).append("svg").attr("width", width).attr("height", height).append("g").attr("transform", "translate(" + width / 2 + "," + height / 2 + ")");
                            var g = svg.selectAll(".arc").data(pie(data)).enter().append("g").each(function (d) {
                            ...
                            gPath.attr("d", arc).each(function (d, i) {
                            ...
                            }).on("click", setFilters).on("mouseover.text", function (d) {
                            ...
                            }).call(tip).on('mouseout', tip.hide);
                            $scope.legendData = legendData;
                            if (data.length > 1) {
                                gPath.on("mouseover.tween", arcTween(outerRadius - 15, 0)).on("mouseout.tween", arcTween(outerRadius - 20, 150));
                            }
                            function setFilters(d) {
                                ...
                                var params;
                                if (!config.filters || (!config.filters[d.data[config.displayKey]] && !config.filters["default"])) {
                                    return;
                                }
                    ...
                    link: function ($scope, element) {
                        ...
                        $scope.$watch("data", function (n, o) {
                            if (n !== o || sortedData.length === 0) {
                                if (n.length != 0) {
                                    var noMissing = _.reject(n, function (datum) {
                                        return datum.key === "_missing";
                                    });
                                    sortedData = _.sortBy(noMissing, function (n) {
                                        return n.key;
                                    });
                                    if (sortedData.length > $scope.maxNumBars) {
                                        ...
                                        sortedData = _.map(chunked, function (chunk) {
                                            ...
                                            return { "doc_count": _.sum(chunk, "doc_count"), "key": keys.length > 1 ? _.min(keys) + "-" + _.max(keys) : _.first(keys) };
                                        });
                                    }
                                    drawBars();
                                }
                                    ...
                                    chart.append("g").append("text").attr("x", 20).attr("y", 20).text("no data to graph");
                                }
                            }
                        });
                        function resizeChart() {
                            if (element.find(".chart-container").is(":visible")) {
                                ...
                                d3.select(element.find(".chart-container > svg")[0]).attr("width", width + margin.left + margin.right).attr("height", $scope.height);
                                drawBars();
                            }
                        }
                        function createChart() {
                            var height = $scope.height;
                            var chart = d3.select(element.find(".chart-container")[0]).append("svg").attr("width", width + margin.left + margin.right).attr("height", height).append("g").attr("transform", "translate(-10,0)");
                        }
                        function drawBars() {
                            var x = d3.scale.ordinal().domain(_.pluck(sortedData, "key")).rangeRoundBands([0, width], 0);
                            var y = d3.scale.linear().domain([0, _.max(_.pluck(sortedData, "doc_count"))]).range([$scope.height, 0]);
                            var chart = d3.select(element.find(".chart-container > svg")[0]);
                            chart.selectAll("g").remove();
                            var elements = chart.selectAll("g").data(sortedData);
                            var tip = d3.tip().attr("class", "tooltip").offset([-5, 0]).html(function (d) {
                                return d.key + ": " + d.doc_count;
                            });
                            var bars = elements.enter().append("g").attr("transform", function (d) {
                                return "translate(" + x(d.key) + ",0)";
                            }).append("rect").attr("y", function (d) {
                                return y(d.doc_count);
                            }).attr("height", function (d) {
                                return $scope.height - y(d.doc_count);
                            }).attr("width", x.rangeBand()).attr("class", "bar").call(tip).on('mouseover', tip.show).on('mouseout', tip.hide);
                        }
                    }
                };
            }
            ...
            function MarkedBarChart($state) {
                return {
                    ...
                    link: function ($scope, element) {
                        var ASPECT_RATIO = 1.33, CLAMP_HEIGHT = Number.POSITIVE_INFINITY, CLAMP_WIDTH = Number.POSITIVE_INFINITY;
                        var _data, _svg, _barChartBoundingBox, _barChartCanvas, _barChartTitle, _barChartLegend, _barChartMarkerGroup, _barChartCaption, _chartMargin, _width, _height, _tipFn, _xScale, _yScale, _xAxis, _xAxisGroup, _colourScale, _postFixID = _getUniqueChartID(8);
                        ...
                        function _initChartSize() {
                            ...
                            if (!_svg) {
                                ...
                                _svg = d3.select(element[0]).select("svg.marked-bar-chart-component").attr("viewPort", "0 0 " + _width + " " + _height);
                                _barChartTitle = _svg.select("g.chart-title-container").append("g").classed("marked-bar-chart-title", true).append("text").attr("y", _chartMargin.top);
                                _barChartLegend = _svg.select("g.chart-title-container").append("g").classed("marked-bar-chart-legend", true);
                                _barChartMarkerGroup = _barChartLegend.append("g").attr("transform", "translate(-25, 55)");
                                _barChartMarkerGroup.append("circle").classed("marked-bar-chart-marker", true).attr("cx", 10).attr("cy", 10).attr("r", 8);
                                _barChartMarkerGroup.append('use').attr('xlink:href', '#bar-chart-marker-symbol').attr("fill", "#fff").attr("x", 115).attr("y", 95).attr("transform", "scale(0.030, 0.030)");
                                _barChartCaption = _barChartLegend.append("text").classed("marked-bar-chart-title-label", true).attr("y", _chartMargin.top + 30);
                                _barChartBoundingBox = _svg.select(".chart-canvas-area > rect");
                                _barChartBoundingBox.attr("x", _chartMargin.left).attr("y", _chartMargin.top + 50);
                            }
                            ...
                            if ($scope.title) {
                                _barChartTitle.text($scope.title).attr("text-anchor", "middle").transition().attr("x", Math.round(_width / 2));
                            }
                            // Calculate the new center of the legend caption if there is one
                            if ($scope.markedLegendLabel) {
                                _barChartCaption.text($scope.markedLegendLabel);
                                //.attr("text-anchor", "middle")
                                _barChartLegend.transition().attr("transform", "translate(" + (Math.round((_width - _barChartLegend.node().getBBox().width) / 2) + _chartMargin.left) + ", 0)");
                            }
                            // Calculate the new bounding box
                            _barChartBoundingBox.attr("width", _width - _chartMargin.left - _chartMargin.right).attr("height", _height - _chartMargin.top - _chartMargin.bottom - 40);
                        }
                        function _renderBars() {
                            ...
                            var barGroups = _barChartCanvas.selectAll("g.rect").data(_data);
                            barGroups.enter().append("g").classed("rect", true);
                            var bars = barGroups.selectAll("rect.bar-item").data(function (d) {
                                return [d];
                            });
                            bars.enter().append("rect").attr("x", 0).attr("y", 0).classed("bar-item", true);
                            barGroups.attr("transform", function (d) {
                                return "translate(" + _xScale(d._key) + ", " + _height + ")";
                            }).transition().delay(function (d, i) {
                                return i / _data.length * 500;
                            }).attr("transform", function (d) {
                                return "translate(" + _xScale(d._key) + ", " + (_height - _yScale(d._count) - _chartMargin.bottom + 10) + ")";
                            });
                            bars.attr("width", _xScale.rangeBand()).attr("height", function (d) {
                                return _yScale(d._count);
                            }).attr("fill", function (d) {
                                d._colour = _colourScale(barCounter++);
                                return d._colour;
                            }).call(_tipFn).on("click", function (d) {
                                var filters = {
                                    "op": "and",
                                    "content": [
                                        {
                                            "op": "in",
                                            "content": { "field": "primary_site", "value": [d._key] }
                                        }
                                    ]
                                };
                                _tipFn.hide(d);
                                ...
                                var bar = d3.select(this), barColour = bar.attr("fill");
                                bar.interrupt().transition().attr("fill", d3.hsl(barColour).darker(1)).attr("transform", "translate(0, 5)");
                                _tipFn.show(d);
                            }).on("mouseout", function (d) {
                                var bar = d3.select(this), barColour = d._colour;
                                bar.interrupt().transition().attr("fill", barColour).attr("transform", "translate(0, 0)");
                                _tipFn.hide(d);
                            });
                            _xAxis = d3.svg.axis().scale(_xScale).orient('bottom').tickSize(0, 0).ticks(0);
                        ...
                        function _initListeners() {
                            ...
                            jQuery(window).on("resize." + _postFixID, _.debounce(function () {
                                _initChartSize();
                                _renderBars();
                            }, 200));
                            $scope.$watch(function () {
                                ...
                                if (!_.isArray(newData) || newData.length === 0 || (newData === oldData && _data)) {
                                    _barChartCanvas.append("g").classed("message", true).append("text").attr("x", Math.round(_width / 2)).attr("y", 200).attr("text-anchor", "middle").text("No Data...");
                                    return;
                                }
                                // Delete any messages
                                _svg.selectAll(".message").remove();
                                _data = _.cloneDeep(newData);
                                _colourScale = d3.scale.category20c().domain([0, _data.length]);
                                _xScale = d3.scale.ordinal().domain(_.map(_data, function (d) {
                                    return d._key;
                                }));
                                _yScale = d3.scale.linear().clamp(true).domain([0, d3.max(_data, function (d) {
                                    return d._count;
                                })]);
                                _xAxisGroup = _svg.append('g').classed('x axis', true).attr('transform', "translate(" + Math.round(_chartMargin.left / 2) + ", " + Math.round(_height - (_chartMargin.bottom / 2)) + ")");
                                _renderBars();
                            });
                        }
                        function _initChart() {
                            ...
                            _tipFn = d3.tip().attr("class", "tooltip").offset([-5, 0]).html(tipFn);
                            _initListeners();
                        }
                        _initChart();
                    }
            ...
            angular.module("components.charts", [
                "location.services"
            ]).directive("chartLegend", ChartLegend).directive("pieChart", PieChart).directive("barChart", BarChart).directive("markedBarChart", MarkedBarChart);
