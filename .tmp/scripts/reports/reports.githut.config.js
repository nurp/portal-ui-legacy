angular.module("reports.githut.config", []).factory("ReportsGithut", function (ReportsGithutColumns, ReportsGithutConfig) {
    return function (data) {
        ...
        types = _.sortBy(types, function (type) {
            return 0 - _.indexOf(order, type.id);
        });
        ...
        ReportsGithutConfig.columns = columns.map(function (c) {
            return c.id;
        });
        ...
        ReportsGithutConfig.dimensions = _.map(_.filter(columns, function (column) {
            return column.dimensional;
        }), function (column) {
            return column.id;
        });
        return {
