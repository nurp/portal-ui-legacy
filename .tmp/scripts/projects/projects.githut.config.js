angular.module('projects.githut.config', []).factory("ProjectsGithut", function (ProjectsGithutConfig, ProjectsGithutColumns) {
    return function (data) {
        var hits = filterData(data.hits);
        var primary_sites = [];
        function findTheThing(array, data_type, propname) {
            return _.find(array, function (type) {
                return type[propname] === data_type;
            });
        }
        ...
        var aggregations = d3.keys(project_ids).reduce(function (a, key) {
            ...
            ProjectsGithutColumns.filter(function (c) {
                return c.is_subtype;
            }).forEach(function (s) {
